...
2. 对单张 DXF 调用 `municipal_qto_inspect_dxf`；对多张 DXF 调用 `municipal_qto_inspect_dxf_batch`，检查图层、实体、文字和未支持实体。
3. 只有在需要生成副本时调用 `municipal_qto_normalize_dxf`；不得覆盖原始图纸。
4. 只有取得人工确认的专业参数后，才调用 `municipal_qto_calculate`；参数数组可包含 `road_sections`、`network_sections`、`retaining_sections` 中的一种或多种。
5. 计算后调用 `municipal_qto_get_job`，完整展示专业、公式、输入、SHA-256、规则版本、告警和审核状态。
6. 旧调用若只有挡护参数，可暂时使用 `municipal_qto_calculate_retaining`；新集成统一使用 `municipal_qto_calculate`。
7. 只有用户明确完成核对并确认后，才调用 `municipal_qto_review_job`；通过前必须勾选原图、设计依据、专业参数、单位/规则和工程部位五项。
8. 复核后再次调用 `municipal_qto_get_job`，报告实际状态：`REVIEWED_PENDING_AUTHORITY`、`FACT_CONFIRMED`、`RETURNED` 或 `REJECTED`。
9. 最终回答必须说明：识别结果是 `Hypothesis`、计算结果是 `Inference`；未通过本地身份核验的审核不得作为 `Fact`。
