...
TOOLS = [
    ...
    _tool("municipal_qto_review_job", "按人工确认清单记录算量作业复核；只有本地已认证审核人才能把 Inference 提升为 Fact。", {"job_id": {"type": "string", "pattern": "^[A-Za-z0-9_-]{4,80}$"}, "reviewer_id": {"type": "string", "description": "审核人标识；正式模式必须与本地已认证身份一致"}, "reviewer_role": {"type": "string", "enum": ["production", "technical", "cost", "project_manager"]}, "decision": {"type": "string", "enum": ["approve", "return", "reject"]}, "checked_items": {"type": "array", "items": {"type": "string", "enum": ["source_drawing", "design_basis", "section_parameters", "units_and_rule", "location_scope"]}}, "note": {"type": "string", "description": "审核备注"}, "evidence_refs": {"type": "array", "items": {"type": "string"}, "description": "项目内证据相对引用"}, "confirm": {"type": "boolean", "description": "必须明确为 true"}}, ["job_id", "reviewer_id", "reviewer_role", "decision", "checked_items", "confirm"], read_only=False, idempotent=False),
]
